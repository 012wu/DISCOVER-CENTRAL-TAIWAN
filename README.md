...
**首頁導覽列**：Logo、首頁、景點、旅宿、餐飲、關鍵字搜尋（開發中）。
...
**景點／旅宿／餐飲管理**：提供查詢與篩選（全部縣市、熱門分類、關鍵字搜尋、排序方式、筆數篩選、資料統計）、每頁 15 筆分頁列表，以及新增、查詢、修改、刪除等完整 CRUD 功能。
...
```bash
git clone https://github.com/012wu/你的專案名稱.git
cd 你的專案名稱
composer install
cp .env.example .env
...
**環境需求**：PHP、Composer、Laravel、MySQL、Node.js（若專案有使用 npm）、Git

## 注意事項

API Key 或其他敏感設定請放在 `.env` 中，不要直接寫入程式碼；並確認 `.gitignore` 已排除 `.env`、`/vendor`、`/node_modules`，避免上傳至 GitHub。

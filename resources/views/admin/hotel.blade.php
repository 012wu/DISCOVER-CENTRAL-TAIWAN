                    ...
                    <form onsubmit="submitEdit(event, {{$item ->id}})">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="hotelLicenseNumber" class="form-control"
                                value="{{ $item->hotelLicenseNumber }}" required>
                        </div>
                        ...
                        <div class=" mb-3">
                            ...
                            <input type="text" name="hotelClassNo" id="editClassNo{{ $item->id }}" class="form-control"
                                value="{{ $item->hotelClassNo }}"
                                onkeyup="lookupClassName('editClassNo{{ $item->id }}', 'editClassName{{ $item->id }}', 'editClassName2{{ $item->id }}')">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">旅宿分類代碼名稱(自動帶入)</label>
                            <input type="text" name="hotelClassName" id="editClassName{{ $item->id }}" class="form-control"
                                value="{{ $item->hotelClassName }}" readonly>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">旅宿分類代碼名稱_new(自動帶入)</label>
                            <input type="text" name="hotelClassName2" id="editClassName2{{ $item->id }}" class="form-control"
                                value="{{ $item->hotelClassName2 }}" readonly>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form onsubmit="submitAdd(event)">
                    ...
                    <div class="mb-3">
                        <label class="form-label">分類代碼(請用 "," 隔開)</label>
                        <input type="text" name="hotelClassNo" id="addClassNo" class="form-control"
                            onkeyup="lookupClassName('addClassNo', 'addClassName', 'addClassName2')">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">旅宿分類代碼名稱(自動帶入)</label>
                        <input type="text" name="hotelClassName" id="addClassName" class="form-control" readonly>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">旅宿分類代碼名稱_new(自動帶入)</label>
                        <input type="text" name="hotelClassName2" id="addClassName2" class="form-control" readonly>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">緯度</label>
...
<script>
    ...
    function lookupClassName(noId, nameId, name2Id) {
        ...
        window.lookupTimer = setTimeout(() => {
            ...
            fetch(`/api/hotelClass/lookup?codes=${codes}`)
                .then(res => res.json())
                .then(result => {
                    if (result.success) {
                        document.getElementById(nameId).value = result.hotelClassName;
                        document.getElementById(name2Id).value = result.hotelClassName2;
                    }
                });
        }, 500);
    }
    ...
    function validatehotelForm(data) {
        if (!data.hotelID) return '景點編號為必填欄位';
        if (!data.hotelName) return '名稱為必填欄位';
        if (!data.hotelClassNo) return '分類代碼為必填欄位';
        if (!data.hotelLicenseNumber) return '旅宿證編號為必填欄位';
